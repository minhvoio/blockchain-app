...
class Contract {
  ...
  constructor() {
    this.token_id = 0;
    this.owner_id = "";
    this.owner_by_id = new LookupMap<string>("o");
    this.token_by_id = new LookupMap<string>("t");
  }
  ...
  init({ owner_id, prefix }: { owner_id: AccountId; prefix: string }) {
    this.token_id = 0;
    this.owner_id = owner_id;
    this.owner_by_id = new LookupMap<string>(prefix);
    this.token_by_id = new LookupMap<string>("t");
  }

  @call({}) // token_id = 0
  mint_nft({ token_owner_id, name, description, media_uri, level }) {
    this.owner_by_id.set(this.token_id.toString(), token_owner_id); //{tokenId = 0, 'minhvo.testnet'}
    ...
    this.token_by_id.set(this.token_id.toString(), token.toString());
    this.token_id++;
